...
def create_simple_nbt_class(tag_id, tag_name, tag_width, tag_parser):
    ...
    class DataNBTTag:
        ...
        def print(self, indent=''):
            print(indent + type(self).clazz_name + ': ' +
                  self.tag_name + ' = ' + str(self.tag_value))
        ...
        def serialize(self, stream, include_name=True):
            if include_name:
                stream.write(type(self).clazz_id.to_bytes(
    ...
    class DataNBTTag:
        ...
        def parse(cls, stream, name):
            payload_length = int.from_bytes(
                stream.read(2), byteorder='big', signed=False)
        ...
        def print(self, indent=''):
            print(indent + 'String: ' + self.tag_name +
                  ' = ' + str(self.tag_value))
        ...
        def serialize(self, stream, include_name=True):
            ...
            stream.write(len(self.tag_value).to_bytes(
...
def create_array_nbt_class(tag_id, tag_name, sub_type):
    class ArrayNBTTag:
        ...
        def parse(cls, stream, name):
            ...
            tag = cls(name)
            for dummy in range(payload_length):
        ...
        def print(self, indent=''):
            print(indent + type(self).clazz_name + ': ' +
                  self.tag_name + ' size ' + str(len(self.children)))
        ...
        def serialize(self, stream, include_name=True):
            ...
            stream.write(len(self.children).to_bytes(
...
def create_list_nbt_class(tag_id):
    class ListNBTTag:
        ...
        def parse(cls, stream, name):
            ...
            sub_type = int.from_bytes(stream.read(
                1), byteorder='big', signed=False)
            payload_length = int.from_bytes(
                stream.read(4), byteorder='big', signed=True)
        ...
        def print(self, indent=''):
            print(indent + 'List: ' + self.tag_name +
        ...
        def serialize(self, stream, include_name=True):
            ...
            stream.write(self.sub_type_id.to_bytes(
                1, byteorder='big', signed=False))
            stream.write(len(self.children).to_bytes(
                4, byteorder='big', signed=True))
...
def create_compund_nbt_class(tag_id):
    class CompundNBTTag:
        ...
        def print(self, indent=''):
            print(indent + 'Compound: ' + self.tag_name +
                  ' size ' + str(len(self.children)))
            for c in self.children:
...
def parse_nbt(stream):
    ...
    tag_name_length = int.from_bytes(
        stream.read(2), byteorder='big', signed=False)
